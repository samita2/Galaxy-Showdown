...
const deadImage = '<img width="75" height="75" title="" alt="" src="data:image/png;base64," />';
...
class MafiaPlayer extends Rooms.RoomGamePlayer {
	...
	event(event) {
		if (this.class[event].target) {
			...
			if (this.class[event].target.type === 'single') {
	...
	kill(message) {
		...
		this.game.announcementWindow(deadImage, message + '<br/>' + this.name + ', the ' + this.class.name + ' lies dead on the ground.');
	...
	eliminate() {
		...
		if (this.game.gamestate === 'pregame') {
			this.game.announcementWindow('', this.name + ' was kicked from the game.');
		} else {
			this.game.announcementWindow(deadImage, this.name + ', the ' + this.class.name + ' was eliminated from the game.');
		}
		delete this.game.players[this.userid];
		this.destroy();
	}
	...
	getRole() {
		this.sendRoom('|html|' + this.game.mafiaWindow(this.class.image, this.class.flavorText));
	}
	...
	targetWindow(image, content) {
		...
		for (let i in this.validTargets) {
			output += '<button value="/mafia target ' + this.validTargets[i].userid + '" name="send">' + this.validTargets[i].name + '</button>';
		}
	...
	updateTarget(image) {
		this.sendRoom('|uhtmlchange|mafia' + this.game.room.gameNumber + 'target' + this.game.gamestate + this.game.day + '|' + this.game.mafiaWindow(image, 'Targeting ' + this.target.name + '!'));
	}
	...
	voteWindow(image, content) {
		...
		for (let i in this.validVotes) {
			output += '<button value="/mafia vote ' + this.validVotes[i].userid + '" name="send">' + this.validVotes[i].name + '</button>';
		}

		console.log("Showing shit to " + this.name);
	...
	singleTarget(side, targetSelf) {
		...
		for (let i in this.game.players) {
			...
			if ((side === 'all' || thisSide === side || (side === 'nomafia' && thisSide !== 'mafia')) && (targetSelf || this.game.players[i] !== this)) {
...
class Mafia extends Rooms.RoomGame {
	...
	updateVotes() {
		...
		for (let i in this.currentVote) {
			text += this.players[i].name + ': ' + this.currentVote[i] + ' votes.<br/>';
		}
		...
		for (let i = 0; i < this.executionOrder.length; i++) {
			...
			if (player.targeting || player.voting) {
				...
				if (player.roleBlocked) {
					...
					let output;
					if (player.target) {
						output = player.toExecute(player.target);
					} else {
						output = player.toExecute();
					}
